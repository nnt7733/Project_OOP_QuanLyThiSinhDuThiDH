...
namespace Chương_trình_quản_lý_thí_sinh_dự_thi_đại_học
{
    public class ThiSinhKhoiC : ThongTinThiSinh, IThiKhoi
    {
        ...
        public void In()
        {
            ...
            base.InThongTin();
            Diem.InDiem();
            Console.WriteLine($"Tổng điểm khối C: {TongDiem():0.00}");
        }

        public Dictionary<string, double> LayTatCaDiemMon() => Diem.LayTatCaDiemMon();

        public void CapNhatDiemTu(DiemThiKHXH diemMoi)
        {
            Diem.GanDiemCoBan(diemMoi.Toan, diemMoi.Van, diemMoi.Anh);
            Diem.GanDiemChuyenSau(diemMoi.Su, diemMoi.Dia, diemMoi.GDCD);
        }

        public override ThongTinThiSinh SaoChep()
        {
            var clone = new ThiSinhKhoiC();
